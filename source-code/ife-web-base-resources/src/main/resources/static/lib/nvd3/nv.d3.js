...
nv.utils.optionsFunc = function(args) {
  ...
  var margin = {top: 0, right: 0, bottom: 0, left: 0}
    ...
    , highlightZero = true
    , rotateLabels = 0
    , rotateYLabel = false	//true --htran46 changed
    , staggerLabels = false
    , isOrdinal = false
    , ticks = null
...
nv.models.multiBar = function() {
  ...
  var margin = {top: 0, right: 0, bottom: 0, left: 0}
    ...
    , groupSpacing = 0.1
    , dispatch = d3.dispatch('chartClick', 'elementClick', 'elementDblClick', 'elementMouseover', 'elementMouseout')
    , valueFormat = d3.format(',.0f')	//htran46 added
    ;
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      /*y   .domain(yDomain || d3.extent(d3.merge(seriesData).map(function(d) { return stacked ? (d.y > 0 ? d.y1 : d.y1 + d.y ) : d.y }).concat(forceY)))
          .range(yRange || [availableHeight, 0]);*/
      y   .domain(yDomain || d3.extent(d3.merge(seriesData).map(function(d) { return (stacked ? (d.y > 0 ? d.y1 : d.y1 + d.y ) : d.y)*1.5 }).concat(forceY)))
      .range(yRange || [availableHeight, 0]);	//htran46 changed
	  ...
	  if (stacked)
          bars.transition()
			...
			.attr('transform', function(d,i,j) { return 'translate(' + x(getX(d,i)) + ',0) rotate(-90 ' + ((stacked ? 0 : (j * x.rangeBand() / data.length )) + 4) + ' ' + (y(stacked ? d.y1 : 0) - 9) + ')'; });
      else
          bars.transition()
            .delay(function(d,i) {
                return i * delay/ data[0].values.length;
			...
			.attr('transform', function(d,i,j) { return 'translate(' + x(getX(d,i)) + ',0) rotate(-90 ' + ((stacked ? 0 : (j * x.rangeBand() / data.length )) + 4) + ' ' + ((getY(d,i) < 0 ?
                        y(0) :
                        y(0) - y(getY(d,i)) < 1 ?
                          y(0) - 1 :
                        y(getY(d,i)) || 0) - 9) + ')'; });
...
nv.models.multiBarChart = function() {
  ...
  yAxis
    .orient((rightAlignYAxis) ? 'right' : 'left')
    //.tickFormat(d3.format(',.1f'))
	.tickFormat(d3.format(',.0f'))	//htran46 changed
    ;
    ...
    selection.each(function(data) {
      ...
      if (showLegend) {
        ...
        g.select('.nv-legendWrap')
        	...
        	.attr('transform', 'translate(' + controlWidth() + ',' + (availableHeight + 10) +')');	// --htran46 changed
      }
          ...
          pieLabels.select(".nv-label text")
                ...
                .text(function(d, i) {
                  ...
                  var labelTypes = {
                    ...
                    "percent": d3.format('.1%')(percent) //Hai Vu Changed
                  };
